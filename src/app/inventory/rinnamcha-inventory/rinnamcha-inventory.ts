...
@Component({
  ...
  templateUrl: './rinnamcha-inventory.html',
  styleUrls: ['./rinnamcha-inventory.scss'],
  encapsulation: ViewEncapsulation.None
})
export class RinnamchaInventory implements OnInit {
  ...
  `).eq('IS_ACTIVE_YN', 'Y').order('ITEM_GROUP').order('SEQ');
   ...
   async getTransactionInventory() {
     ...
     const {data, error} = await this.supabase
       ...
       .select(`
         M_INVENTORY_RINNAMCHA_ID,
         AMOUNT,
         M_INVENTORY_RINNAMCHA!inner(
           ITEM_GROUP
         )
       `)
     ...
     if (this.inventories.length > 0) {
       this.inventories.controls.forEach(group => {
         const transaction = data?.find((item: any) => item.M_INVENTORY_RINNAMCHA_ID === group.get('master_id')?.value);
     ...
     if (data && Array.isArray(data) && data.length > 0) {
       ...
       data.forEach((item: any) => {
         ...
         if (Array.isArray(item.M_INVENTORY_RINNAMCHA)) {
           itemGroup = item.M_INVENTORY_RINNAMCHA[0]?.ITEM_GROUP;
         } else if (typeof item.M_INVENTORY_RINNAMCHA === 'object' && item.M_INVENTORY_RINNAMCHA !== null) {
           itemGroup = item.M_INVENTORY_RINNAMCHA.ITEM_GROUP;
         }
  ...
  async getTodayRemark() {
    const today = this.getDate();
    const {data, error} = await this.supabase
      .from('T_INVENTORY_RINNAMCHA_REMARK')
      .select('REMARK')
      .gte('CREATED_DATETIME', `${today}T00:00:00`)
      .lte('CREATED_DATETIME', `${today}T23:59:59`)
  ...
  async confirmSave() {
    ...
    try {
      ...
      const inventoryInsertData: InventoryInsertModel[] = this.inventories.controls
        ...
        .map(ctrl => ({
          M_INVENTORY_RINNAMCHA_ID: ctrl.get('master_id')?.value,
      ...
      const idsToDelete = inventoryInsertData.map(i => i.M_INVENTORY_RINNAMCHA_ID);
   ...
   * records for those M_INVENTORY_RINNAMCHA_IDs; otherwise delete all records for today (legacy behaviour).
   */
  async insertTInventory(inventoryInsertData: InventoryInsertModel[], idsToDelete?: any[]) {
    ...
    let deleteQuery = this.supabase.from('T_INVENTORY_RINNAMCHA').delete();
    if (Array.isArray(idsToDelete) && idsToDelete.length > 0) {
      deleteQuery = deleteQuery.in('M_INVENTORY_RINNAMCHA_ID', idsToDelete);
    }
  ...
  // async getTransactionInventory() {
  //   const {data, error} = await this.supabase
  //     .from<'T_INVENTORY_RINNAMCHA', InventoryModel>('T_INVENTORY_RINNAMCHA') // ✅ T = table name, R = row type
  //     .select(`
  //   ID,
  //   AMOUNT,
  //   M_INVENTORY_RINNAMCHA!inner(ID, DESC)
  // `);
  //   console.log(data);
  //   if (error) throw error;
  //
  //   if (data) {
  //     data?.forEach(inv => {
  //       const masters = Array.isArray(inv.M_INVENTORY_RINNAMCHA) ? inv.M_INVENTORY_RINNAMCHA : [inv.M_INVENTORY_RINNAMCHA];
